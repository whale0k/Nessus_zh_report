...
│   └── word.py                  # Word 报告生成器
├── target/                      # 扫描文件目录（放入 .nessus / .csv 文件）
├── main.py                      # 主程序入口
├── README.md                    # 本文件
├── requirements.txt             # Python 依赖
└── nessus_plugins.db            # SQLite 缓存数据库（自动生成）
```
...
- **API 域名**: `www.tenablecloud.cn`（简体接口）、zh-tw.tenable.com（繁体中文接口）
...
程序采用**三级降级策略**自动获取漏洞的中文信息，确保在各种网络环境下都能生成中文报告：

```
漏洞 plugin_id
    │
    ▼
① 本地数据库查询 (nessus_plugins.db)
    │
    ├─ 命中 → 返回中文数据 ✅
    │
    ▼ 未命中
② Tenable 中文API (CN简体 → TW繁体→简体转换)
    │
    ├─ 命中 → 保存到数据库 → 返回中文数据 ✅
    │
    ▼ 未命中
...
- 查询本地 `nessus_plugins.db` 数据库
- 首次获取到的中文漏洞信息会自动缓存，后续相同漏洞直接读取
- **优点**：速度最快，离线可用

### 第二级：Tenable 官方中文 API

- 优先请求**简体中文 API**（`tenable.com`），失败后降级请求**繁体中文 API**（`tenable.com/zh-tw`）
- 繁体结果通过内置映射表自动转换为简体
- 获取到的数据自动存入本地数据库供后续使用
- **优点**：官方数据，准确度高

### 第三级：在线翻译服务（兜底）

- 当前两级均无法获取时，调用在线翻译将英文文本翻译为中文
- 翻译源优先级可在 `config.ini` 中配置，默认 `bing,google`
- 翻译结果**默认不保存**到数据库（可通过 `保存翻译结果 = yes` 开启）
- **优点**：覆盖面广，确保中文输出

> **注意**：翻译仅针对用户选择的漏洞等级范围执行。例如选择"高危及以上"时，只会翻译 Critical 和 High 级别的漏洞。
