...
class WordReportGenerator:
    ...
    def _calculate_statistics(
        ...
        if risk_counts is not None and host_risk_counts is not None and total_hosts is not None:
            ...
            total_vulns = sum(v for k, v in risk_counts.items() if k != "Info")
        ...
        try:
            ...
            for i, paragraph in enumerate(doc.paragraphs):
                if placeholder in paragraph.text:
                    ...
                    self._remove_following_page_break(doc, i)
    ...
    @staticmethod
    def _remove_following_page_break(doc: Document, index: int) -> None:
        """Remove the paragraph immediately following the given index if it contains only a page break"""
        ...
        for run in next_para.runs:
            for child in run._element:
                if child.tag.endswith('}br') and child.get('{http://schemas.openxmlformats.org/wordprocessingml/2006/main}type') == 'page':
                    ...
                    break
        if has_page_break:
            text_stripped = next_para.text.strip()
            runs_have_content = any(run.text.strip() for run in next_para.runs if not (run._element.findall('.//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}br')))
            if not text_stripped or not runs_have_content:
                p_element = next_para._element
                p_element.getparent().remove(p_element)
                logger.info(f"Removed page break paragraph after index {index}")
